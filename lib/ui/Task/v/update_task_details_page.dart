...
class _PageState extends StateMVC<UpdateTaskDetailsPage> {
  ...
  Widget _ApplicationPackage() {
    ...
    if(TimeCompletion.value.text.isEmpty){
      TimeCompletion.text=(double.parse(model.accelerator.toString())/0.020).toStringAsFixed(0);
    }
      return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
              ...
              child: InputWidget(
                ...
                onChanged: (v) {
                  if(TimeCompletion.value.text.isNotEmpty) {
                    setState(() {
                      //Like this?
                      //yeah need volume here instead of accelerator

                      //model.accelerator = (double.parse(model.volume) * 0.020).toStringAsFixed(2);
                      model.accelerator = ((((double.parse(TimeCompletion.value
                          .text.toString()) / 100)/1425)*350)* double.parse(model.volume)).toStringAsFixed(1);
